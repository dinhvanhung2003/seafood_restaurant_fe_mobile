// src/components/modals/CancelOneItemModal.native.tsx
      ...
      <View style={tw`flex-1 bg-black/40 items-center justify-center`}>
        <View style={tw`w-[90%] rounded-xl bg-white p-5`}>
          ...
          <Text style={tw`text-sm text-slate-600 mb-4`}>
            Bạn có chắc chắn muốn huỷ món <Text style={tw`font-semibold`}>{item.name}</Text> không?
          </Text>
            ...
            <View style={tw`flex-row flex-wrap gap-2`}>
              {REASONS.map((r) => {
                ...
                return (
                  ...
                  >
                    <Text style={tw`${active ? 'text-white' : 'text-slate-700'} text-sm`}>{r}</Text>
                  </Pressable>
                );
              })}
            </View>
          ...
          <View style={tw`flex-row justify-end gap-2`}>
            <Pressable onPress={onClose} style={tw`h-10 px-4 rounded-md border items-center justify-center`}>
              <Text>Huỷ</Text>
            </Pressable>
            <Pressable
              disabled={!canSubmit}
              onPress={async () => canSubmit && (await onConfirm({ qty, reason }))}
              style={tw.style(
                'h-10 px-4 rounded-md items-center justify-center',
                canSubmit ? 'bg-red-600' : 'bg-red-300',
              )}
            >
              <Text style={tw`text-white font-semibold`}>Chắc chắn</Text>
            </Pressable>
          </View>
        </View>
      </View>
